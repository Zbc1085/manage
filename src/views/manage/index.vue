    ...
    <div class="manage tc">
        <button v-on:click="add">新增</button>
         ...
         <table>
           ...
           <tr v-for="(item,index) in pepoles">
             <td>
               {{item.name}}
             </td>
       ...
       methods:{
         ...
         addName(){
           ...
           if(v.trim()==""){
             ...
             var data={}
             data.name=v
             this.pepoles.push(data)
           }
          },
         del(e){
             var id=e.target.offsetParent.id
             this.pepoles.splice(id,1)
         },
         edit(e){
            this.showEdit=true
            var id=e.target.offsetParent.id
            this.editId=id
            this.newName=this.pepoles[id].name
         },
         cancel(){
            this.showEdit=false
         },
         editName(){
            ...
            if(v.trim()==""){
                ...
                this.pepoles[this.editId].name=this.newName
                this.showEdit=false
            }
         }
       }
